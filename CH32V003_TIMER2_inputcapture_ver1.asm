...
#	li x10,result1		# set pointer x10 to SRAM register mem to store the byte
	addi x10,x10,16		# mem is 16 bytes ahead of result1
...
#	addi x10,x10,16
	li t1,10		# byte counter loaded with 10 , total 10 bytes to be printed

Z:
	lb x8,0(x10)		# load byte from mem
	
print1:
	addi x10,x10,1		# increase the address by 1 byte
	addi t1,t1,-1		# decrease the byte counter
	call USART_TX		# call uart
	bnez t1,Z		# if counter not 0 loop the read process till counter is 0
	li x8,0x0d		# line feed
	call USART_TX		# call uart
	li x8,0x0a		# carriage feed
	call USART_TX		# call uart
