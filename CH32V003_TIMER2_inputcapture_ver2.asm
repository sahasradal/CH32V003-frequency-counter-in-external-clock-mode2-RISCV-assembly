# works fine but output is hex in terminal-period and frequency, uses update interrupt and input capture interrupt
...
timer_for_1s:
	li x10,R16_TIM2_PSC
	li x11,(7)			# fck_PSC/PSC[15:0]+1,8MHz/7+1 =1MHz , 1ms resolution for each click
	sw x11,0(x10)
	...
	lw x11,0(x10)
	li x7,((0<<1)|(1<<2)|(1<<7))	# UDIS =0 ,URS =1(update only when overflow),ARPE=1(ARR updated on event)
	or x11,x11,x7
	sw x11,0(x10)
...
###########
	...
	mv x8,x11			# copy to x8
	li x7,0x01			# UIF bit mask
	and x8,x8,x7			# and the x8 copy with UIF bit mask
	beq x8,x7,update		# if x8 = 1 branch to label "update" , interrupt is an update of counter overflow
	mv x8,x11			# move the copy of flag register to x8 again
	li x7,0x4			# load capture compare interrupt flag mask to x7
	and x8,x8,x7			# and the x8 copy with CC2IF bit mask
	beq x8,x7,incapt2		# if x8 = 4 branch to label "incapt2"as the interrupt is a capture compare.
update:
	li x10,0x20000018 		# overflow register address in sram
	lw x11,0(x10)			# copy overflow register value to x11
	li x7,50000			# increment 1 count of overflow, counter overflows at arr value 50000 (50msx20 = 1second = 1000000 clicks)
	add,x11,x11,x7			# add the max over flow value = ARR to x11
	sw x11,0(x10) 			# overflow register address in sram , store 
	li x10,R16_TIM2_INTFR		# load address of timer2 interrupt flag
	lw x11,0(x10)			# copy word from flag register
	li x7,0xFFFFFFFE		# 0<<UIF
	and x11,x11,x7			# and x11 with UIF mask
	sw x11,0(x10)			# store in flag register
	j nexttime			# exit ISR by jumping to label nexttime
incapt2:
	li x10,R16_TIM2_INTFR		# load address of timer2 interrupt flag
	lw x11,0(x10)			# copy word from flag register
	li x7,0xFFFFFBFD		# 0<<CC2OF| 0<<CC2IF
	...
	li x10,overflow			# overflow has the total overflow clicks
	lw x7,0(x10)			# x11 has the overflow value
	add x7,x7,x11			# add 1st time stamp with total oveerflow
	li x10,result2			# if state register is not 0 load address of result2 as the current interrupt is 2nd time stamp
	sw x7,0(x10)			# store capture register value(2nd time stamp) in result2
	li x10,result1			# load address of result1 in x10
	lw x11,0(x10)			# copy result1 to x7
	sub x7,x7,x11			# subtract x7(result1) from x11(result2) which gives period
	sw x7,0(x10)			# difference in result1
...
result1str:
	...
	sw x7,0(x10)			# x11 has the 1st time stamp + overflow, store in result1
	...
	j nexttime			# exit ISR by jumping to label nexttime
out:	
	li x8,'I'			# reach here only after reading the 2nd time stamp, load ascii I in t2
...
division:
	...
	li a1,1000000			# divident x11 (1000000 us)
